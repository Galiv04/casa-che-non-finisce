...
const Engine = (() => {
  ...
  function resolveChoice(scene, c) {
    ...
    if (c.heal) { for (const h of G.party) if (!h.down && !h.morto) h.hp = Math.min(h.maxHp, h.hp + c.heal); }
    if (c.damage) { for (const h of G.party) if (!h.down && !h.morto) h.hp = Math.max(1, h.hp - c.damage); }
